int sum() {
  List<GameMax> gameMaxes = [];
  List<String> lines = input.split('\n');
  for (var line in lines) {
    gameMaxes.add(createGameMax(line));
  }
  int sum = 0;
  for (var gameMax in gameMaxes) {
    sum += gameMax.maxGreen * gameMax.maxBlue * gameMax.maxRed;
  }
  return sum;
}

bool isPossibleGame(GameMax gameMax) {
  int maxRed = 12;
  int maxGreen = 13;
  int maxBlue = 14;
  return gameMax.maxRed <= maxRed && gameMax.maxBlue <= maxBlue && gameMax.maxGreen <= maxGreen;
}

GameMax createGameMax(String line) {
  /// Game 81: 11 green, 5 blue; 1 blue, 10 green, 1 red; 3 red, 3 blue, 15 green
  int maxRed = 0;
  int maxBlue = 0;
  int maxGreen = 0;
  int id = int.parse(line.split(":")[0].split(" ")[1]); // 81
  String gameData = line.split(":")[1]; // 11 green, 5 blue; 1 blue, 10 green, 1 red; 3 red, 3 blue, 15 green
  List<String> games = gameData.split(";");
  for (var singleGame in games) {
    List<String> colors = singleGame.split(",");
    for (var color in colors) {
      if (color.contains("red")) {
        int red = int.parse(color.split(" ")[1]);
        if (red > maxRed) {
          maxRed = red;
        }
      } else if (color.contains("blue")) {
        int blue = int.parse(color.split(" ")[1]);
        if (blue > maxBlue) {
          maxBlue = blue;
        }
      } else if (color.contains("green")) {
        int green = int.parse(color.split(" ")[1]);
        if (green > maxGreen) {
          maxGreen = green;
        }
      }
  ...
  print(GameMax(id, maxRed, maxBlue, maxGreen).toString());
  return GameMax(id, maxRed, maxBlue, maxGreen);
}
...
final class GameMax {
  ...
  int maxBlue;
  int maxGreen;
  GameMax(this.id, this.maxRed, this.maxBlue, this.maxGreen);

  @override
  String toString() {
    return "GameMax(id: $id, maxRed: $maxRed, maxBlue: $maxBlue, maxGreen: $maxGreen)";
  }
}
