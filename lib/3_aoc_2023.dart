  ...
  for (int row = 0; row < lines.length; row++) {
    ...
    for (int col = 0; col < lineLength; col++) {
      ...
      if (RegExp(r'[0-9]').hasMatch(character)) {
        ...
        if (start != -1) {
          int value = int.parse(lines[row].substring(start, col));
          numberPositions.add(NumberPosition(value, row, start, col - start));
          start = -1;
        }
      }
      if (!RegExp(r'[0-9]').hasMatch(character) && character != '.') {
        characterPositions.add(CharacterPosition(lines[row][col], row, col));
      }
    }
    if (start != -1) {
      int value = int.parse(lines[row].substring(start, lineLength));
      numberPositions
          .add(NumberPosition(value, row, start, lineLength - start));
    }
  }
  var data = findMultipliers(numberPositions, characterPositions);
  int sum = data.$1;
  return sum;
}

(int sum, Set<NumberPosition> numbers) findMultipliers(
    List<NumberPosition> numberPositions,
    List<CharacterPosition> characterPositions) {
  int sum = 0;
  var allMatches = <NumberPosition>{};
  for (var character in characterPositions) {
    if (character.value == "*") {
      var matches = <NumberPosition>{};
      for (var number in numberPositions) {
        if (isNeighbour(number, character)) {
          matches.add(number);
        }
      }
      if (matches.length == 2) {
        ...
        allMatches.addAll(matches.toList());
      }
    }
  }
...
Set<NumberPosition> findNeighbours(
    List<NumberPosition> numbers, List<CharacterPosition> characters) {
  var neighbours = <NumberPosition>{};
...
bool isNeighbour(NumberPosition number, CharacterPosition character) {
  int rowDistance = number.row - character.row;
  for (int col = number.col; col < number.col + number.length; col++) {
    int colDistance = col - character.col;
    if (rowDistance.abs() < 2 && colDistance.abs() < 2) {
      return true;
    }
  }
  return false;
...
/// Defines top left corner of a number in the grid and its length
final class NumberPosition {
  final int value;
  final int row;
  final int col;
  final int length;
  const NumberPosition(this.value, this.row, this.col, this.length);

  @override
  String toString() {
    return 'value: $value, row: $row, col: $col, length: $length';
  }
}

final class CharacterPosition {
  final String value;
  final int row;
  final int col;
  const CharacterPosition(this.value, this.row, this.col);
...
const String longInput =
    r'''.......855.442......190..................................969..........520.......59.............................................172..........
